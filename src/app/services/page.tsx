...
import { upcomingServices } from "@/lib/placeholder-data";
    ...
    <div className="flex flex-col gap-8">
      ...
      {!isLoading && services && services.length === 0 && (
        <div className="flex flex-col items-center justify-center text-center p-10 border-2 border-dashed rounded-lg bg-muted/50">
            ...
            <p className="text-sm text-muted-foreground">Create a new service to get started.</p>
        </div>
      )}
          ...
          <Link href={`/services/${service.id}`} key={service.id}>
            <Card className="flex flex-col bg-transparent shadow-none border-0 h-full group">
              ...
              <CardHeader className="p-2 pt-4">
                ...
                <CardDescription className="text-xs">{new Date(service.date).toLocaleDateString()}</CardDescription>
              </CardHeader>
            </Card>
          </Link>
        ))}
