  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(
      ...
      async (firebaseUser) => {
        ...
        // Immediately update auth state to unblock the UI
        setUserAuthState({
          user: firebaseUser,
          isUserLoading: false,
          userError: null,
        ...
        if (firebaseUser) {
          ...
          // Non-blocking check and creation of user documents
          getDoc(userProfileRef)
            .then((userProfileSnap) => {
              if (!userProfileSnap.exists()) {
                // Profile doesn't exist, create it and the team_member doc.
                const profileData = {
                ...
                setDoc(userProfileRef, profileData).catch((error) =>
                  console.error(
                    'FirebaseProvider: Error creating user profile:',
                    error
                  )
                );
                ...
                setDoc(teamMemberDocRef, teamMemberData).catch((error) =>
                  console.error(
                    'FirebaseProvider: Error creating team member document:',
                    error
                  )
                );
              }
            ...
            .catch((error) =>
              console.error(
                'FirebaseProvider: Error checking user profile:',
                error
              )
            );
        }
