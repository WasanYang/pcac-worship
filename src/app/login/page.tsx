...
const backgroundImageUrl =
  placeholderImages.find((p) => p.id === 'homeBanner')?.imageUrl ||
  '/placeholder.jpg';
...
export default function LoginPage() {
  ...
  useEffect(() => {
    // Redirect only when loading is complete and user object exists
  ...
  // Prevent rendering the login UI while checking auth state or if user is already logged in
  if (isUserLoading || user) {
    ...
    <div className='relative flex min-h-screen w-full flex-col items-center justify-center overflow-hidden bg-background text-foreground'>
      ...
      <AnimatePresence mode='wait'>
            ...
            <div className='relative h-1/2 flex flex-col items-center justify-center text-white'>
              ...
              <div className='z-10 text-center p-4'>
                ...
                <p className='mt-2 text-lg text-white/90'>
                  Streamline your worship team management.
                </p>
              </div>
            </div>
        ...
        {(view === 'signIn' || view === 'signUp') && (
          ...
          >
            {view === 'signIn' ? (
              <SignInForm onGoToRegister={() => setView('signUp')} />
            ) : (
              <SignUpForm onGoToSignIn={() => setView('signIn')} />
            )}
          </motion.div>
        )}
      </AnimatePresence>
